...
export default function () {
  ...
  const [linguaggioAttuale, setLinguaggioAttuale] = useState(languages[0]);

  const buttonClick = (language) => {
    setLinguaggioAttuale(language);
  };
  ...
  return (
    <main>
      ...
      <div className="container">
        <div className="card">
          <h3>{linguaggioAttuale.title}</h3>
          <p>{linguaggioAttuale.description}</p>
        </div>
      </div>
    </main>
  );
